...
export default function SettlementPage() {
  ...
  return <main className="dashboard-shell settlement-shell">
    ...
    <section className="company-section">
      <div className="section-head"><div><h2>{month.replace("-", "년 ")}월 거래처별 정산</h2><p>등록 당시 식수 단가를 우선 적용하고, 기록에 단가가 없으면 현재 거래처 단가를 적용합니다.</p></div></div>
      {loading ? <div className="empty-state"><strong>정산 내역을 불러오고 있습니다.</strong></div> : rows.length === 0 ? <div className="empty-state"><strong>이 달에 정산할 식수 기록이 없습니다.</strong><span>키오스크 식수가 등록되면 자동으로 집계됩니다.</span></div> : <div className="settlement-table-wrap"><table className="settlement-table"><thead><tr><th>거래처</th><th>중식</th><th>중식 금액</th><th>석식</th><th>석식 금액</th><th>총 식수</th><th>청구금액</th></tr></thead><tbody>{rows.map((r) => <tr key={r.id}><td><strong>{r.name}</strong><small>업체번호 {r.company_no}</small></td><td>{r.lunchCount.toLocaleString()}명</td><td>{r.lunchAmount.toLocaleString()}원</td><td>{r.dinnerCount.toLocaleString()}명</td><td>{r.dinnerAmount.toLocaleString()}원</td><td>{r.totalCount.toLocaleString()}명</td><td><strong>{r.totalAmount.toLocaleString()}원</strong></td></tr>)}</tbody><tfoot><tr><td>합계</td><td>{totals.lunch.toLocaleString()}명</td><td></td><td>{totals.dinner.toLocaleString()}명</td><td></td><td>{totals.count.toLocaleString()}명</td><td><strong>{totals.amount.toLocaleString()}원</strong></td></tr></tfoot></table></div>}
    </section>
    <section className="company-section settlement-next"><div><h2>다음 단계</h2><p>정산 금액 검토 후 거래처별 PDF 정산서 생성과 이메일 발송 기능을 연결할 예정입니다.</p></div><button className="btn primary" disabled>PDF 정산서 · 다음 단계</button></section>
  </main>;
}

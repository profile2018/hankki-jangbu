...
export default function KioskPage() {
  ...
  if (done) return <main className={`kiosk-shell done-${mealType}`}><section className="kiosk-card kiosk-done"><div className="done-mark">✓</div><h1>등록되었습니다</h1><p>{company?.company_name} · {mealType === "lunch" ? "중식" : "석식"} {headcount}명</p><strong className="meal-wish">맛있는 식사하세요.</strong><span>잠시 후 PIN 입력 화면으로 돌아갑니다.</span></section></main>;
  ...
  return <main className="kiosk-shell"><section className="kiosk-card">
    ...
    {!company ? <>
      ...
      <div className="meal-buttons">
        <button className={`lunch ${mealType === "lunch" ? "active" : ""}`} onClick={()=>setMealType("lunch")}><span>중식</span><strong>{Number(company.lunch_price).toLocaleString()}원</strong></button>
        <button className={`dinner ${mealType === "dinner" ? "active" : ""}`} onClick={()=>setMealType("dinner")}><span>석식</span><strong>{Number(company.dinner_price).toLocaleString()}원</strong></button>
      </div>
      <div className="headcount-box">
        <span>인원수</span>
        <div className="quick-counts">{[1,2,3,4,5].map((n)=><button key={n} className={headcount===n?"active":""} onClick={()=>setHeadcount(n)}>{n}명</button>)}</div>
        <div className="count-stepper"><button onClick={()=>setHeadcount(Math.max(1, headcount-1))}>−</button><strong>{headcount}명</strong><button onClick={()=>setHeadcount(Math.min(999, headcount+1))}>＋</button></div>
      </div>

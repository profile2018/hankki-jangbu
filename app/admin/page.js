...
export default function SuperAdminPage(){
  ...
  return <main className="super-admin-shell">
    ...
    <section className="super-admin-panel"><div className="section-head"><div><h2>가입 식당</h2><p>현재 서비스 중이거나 이용 정지 상태인 식당입니다.</p></div></div><div className="super-admin-table-wrap"><table className="super-admin-table"><thead><tr><th>식당</th><th>대표자</th><th>가입일</th><th>무료체험</th><th>거래처</th><th>상태</th><th>관리</th></tr></thead><tbody>
      ...
      {!visibleRows.length&&<tr><td colSpan="7" className="admin-empty">현재 가입 식당이 없습니다.</td></tr>}
    </tbody></table></div></section>

    <section className="super-admin-panel archived-panel"><div className="section-head"><div><h2>보관된 식당</h2><p>삭제/보관 처리한 식당입니다. 데이터는 보존되며 필요할 때 복구할 수 있습니다.</p></div><span className="archive-count">{archivedRows.length}곳</span></div><div className="super-admin-table-wrap"><table className="super-admin-table"><thead><tr><th>식당</th><th>대표자</th><th>보관일</th><th>거래처</th><th>관리</th></tr></thead><tbody>
      {archivedRows.map(r=><tr key={r.id}><td><strong>{r.name}</strong><small>{r.phone||r.email||"연락처 미등록"}</small></td><td>{r.owner_name||"-"}</td><td>{dateText(r.archived_at)}</td><td>{r.company_count||0}곳</td><td><button className="admin-manage-btn restore" onClick={()=>restoreRestaurant(r)} disabled={busy}>복구</button></td></tr>)}
      {!archivedRows.length&&<tr><td colSpan="5" className="admin-empty">보관된 식당이 없습니다.</td></tr>}
    </tbody></table></div></section>
